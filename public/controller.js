...
kastor.controller('mainController', ['$scope', '$timeout', 'RouteService', function($scope, $timeout, RouteService){
    ...
    var refreshRoute = function(response) {
        ...
        var totalTimeInHours = runTimeInHours($scope.route.data.trackPoints[0].timeStamp, $scope.route.data.trackPoints[trackPoints.length-1].timeStamp);
    ...
    $scope.playRoute = function() {
        ...
        var step = function() {
            $scope.cursor = {
                ...
                climb: $scope.playbackMultiplier,
